...
class App:
    def __init__(self,root,title) -> None:
        ...
        self.canvas = Canvas(self.root, width=1000, height=600, highlightbackground='darkorange',
                             bg='midnightblue', highlightthickness=2)
        self.canvas.grid(row=1, columnspan=15)
        
        self.N = 50
        self.data = linspace(5, 400, self.N)
        self._colors = ['darkorange'] * self.N
        self._speed = 5 / 1000
        self._shuffle()

    def __reset_colors(self,color ='darkorange'):
        ...
        for steps in range(self.N-1):
            ...
            for i in range(a):
                ...
                if self.data[i]>self.data[i+1]:
                    self._colors[:a+1] = ['darkorange']*(a+1)
                    ...
                    self._display(self._colors)
                    time.sleep(self._speed)
                self._colors[i] = self._colors[i+1] = 'darkorange'
...
if __name__ == '__main__':
    window = Style(theme = 'solar').master  
