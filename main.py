...
class window: 
    ...
    def __init__(self,root,title) -> None:
        ...
        ttk.Style().configure("Custom.Border.TButton", background="black", foreground="white", bordercolor="purple")
        start_btn = ttk.Button(self.root, text='Start', style='Custom.Border.TButton', padding=5, width=15,
                        command=self.start)
        ...
        ttk.Style().configure("Custom.Outline.TButton", background="black", foreground="yellow", bordercolor="yellow")
        ttk.Button(self.root, text='Shuffle', style='Custom.Outline.TButton', padding=5, width=15,
        ...
        self.arraysize=ttk.Scale(self.root,from_=6,to=50,length=300,style='success.Horizontal.TScale',value=10,
            command=lambda x:self.slide_function())
        self.arraysize.grid(row=2,column=1,columnspan=3)
        self.canvas=Canvas(self.root, width=800-5, height=400,highlightbackground="darkorange",highlightthickness=2,
        bg='black')
        self.canvas.grid(row=4, padx=5, pady=10, columnspan=6)

        self.speed=0.2 
        self.N=7
        self.colours=['darkorange' for i in range(self.N)]
        self.shuffle() 

        self.__sorted_array = ['lime' for _ in range(self.N)]
        self.__default_colours = ['darkorange' for i in range(self.N)]
    ...
    def display(self, N: int, a: list, rong: list) -> None:
        ...
        for i in range(N):
            color = rong[i] if i < len(rong) else 'green'
            self.canvas.create_rectangle(7 + i * width + i * gap, 0,
                                         7 + (i + 1) * width + i * gap, a[i], fill=color)
    ...
    def slide_function(self):
        ...
        self.colours=['darkorange' for _ in range(self.N)]
        self.shuffle()
        ...
        if self.sorts['bubble'] is True:
            for i in range(self.N-1):
                for j in range(self.N-1-i):
                        self.display(self.N,self.data,['purple' if a==j or a==j+1 else 'green' if a>self.N-1-i else 'darkorange' for a in range(self.N)])
                        time.sleep(self.speed)
                        if self.data[j]>self.data[j+1]:
                            self.display(self.N,self.data,['red' if a==j or a==j+1 else 'green' if a>self.N-1-i else 'darkorange' for a in range(self.N)])
                            time.sleep(self.speed)
                            self.data[j],self.data[j+1]=self.data[j+1],self.data[j]
                            self.display(self.N,self.data,['lime' if a==j or a==j+1 else 'green' if a>self.N-1-i else 'darkorange' for a in range(self.N)])
                            time.sleep(self.speed)
            ...
            for j in range(1,len(self.data)):
                key=self.data[j]
                i=j-1
                self.display(self.N,self.data,['purple' if a==i or a==i+1 else 'green' if a<=j else'darkorange' for a in range(self.N)])
                time.sleep(self.speed)
                while i>=0 and self.data[i]>key:
                        self.data[i+1]=self.data[i]
                        self.display(self.N,self.data,['yellow' if a==i else 'green' if a<=j else'darkorange' for a in range(self.N)])
                        time.sleep(self.speed)
                        i-=1
                self.data[i+1]=key
            self.display(self.N,self.data,self.__sorted_array)
            ...
            for i in range(len(self.data)-1):
                ...
                for j in range(i+1,len(self.data)):
                        self.display(self.N,self.data,['yellow' if a==min_index or a==i else 'green' if a<=i else 'darkorange' for a in range(self.N)])
                        time.sleep(self.speed)
                        if self.data[min_index]>self.data[j]:
                            self.display(self.N,self.data,['red' if a==min_index or a==j else 'green' if a<=i else 'darkorange' for a in range(self.N)])
                            time.sleep(self.speed)
                            min_index=j
                if min_index!=i:
                        self.data[i], self.data[min_index]=self.data[min_index],self.data[i]
                        self.display(self.N,self.data,['lime' if a==min_index or a==i else 'green' if a<=i else 'darkorange' for a in range(self.N)])
                        time.sleep(self.speed)
            self.display(self.N,self.data,self.__sorted_array)
            ...
            messagebox.showerror("Visualizer", "Please select any algorithm")
    ...
    def partition(self,a,i,j):
        ...
        while i<j:
            while  i<len(a) and a[i]<= pivot:
                i+=1
                self.display(self.N,self.data,['purple' if x==piv_index else 'yellow' if x==i else "darkorange" for x in range(self.N)])
                time.sleep(self.speed)
            while a[j]>pivot:
                j-=1
            if i<j:
                self.display(self.N,self.data,['red' if x==i or x==j else "darkorange" for x in range(self.N)])
                time.sleep(self.speed)
                a[i],a[j]=a[j],a[i]
                self.display(self.N,self.data,['lime' if x==i or x==j else "darkorange" for x in range(self.N)])
                time.sleep(self.speed)
        a[j],a[l]=a[l],a[j]
        return j
...
if __name__ == '__main__':
     win = Style(theme='solar').master
     obj = window(win, 'Visualizer : Sorting Algorithms')
